...
def lambda_handler(event, context):
    """
    Funtion to validate product direct client in LMK
    """
    ...
    if event["validationResult"]["continueValidation"] is True:
        ...
        sf_landmark_id = event.get("sfAdAgAccountLandmarkId", "")
        ...
        if (
            ...
            description = f"Advertiser [{event['brqClientName']}] is not the Payer allocated on the Product [{event['lmkProductResponse']['productName']}]. The Biller allocated is [{event['lmkProductResponse']['payerName']}]"
            ...
            event["validationResult"]["details"].append(
                {
                    "ruleName": "Validate Product direct client in LMK",
